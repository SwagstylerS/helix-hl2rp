...
function Schema:SaveCheckpoints()
	...
	for _, v in ipairs(ents.FindByClass("ix_checkpoint")) do
		data[#data + 1] = {v:GetPos(), v:GetAngles(), v:GetMode(), v:GetCheckpointName()}
	end

	ix.data.Set("checkpointFields", data)
end

function Schema:LoadCheckpoints()
	for _, v in ipairs(ix.data.Get("checkpointFields") or {}) do
		local checkpoint = ents.Create("ix_checkpoint")

		checkpoint:SetPos(v[1])
		checkpoint:SetAngles(v[2])
		checkpoint:Spawn()
		checkpoint:SetMode(v[3])
		checkpoint:SetCheckpointName(v[4] or "Checkpoint")
	end
end
